...
The $X_i$s here are typically not i.i.d, the distributions change as the sequence progresses.
...
Let $X_1, X_2, \dots$ be i.i.d with $EX_i = \mu$ and $\text{Var}(X_i) = \sigma^2 < \infty$. Define $\overline X = 1/n \sum_{i=1}^n X_i$. Then for every $\epsilon > e$,
...
Let $X_1,X_2, \dots$ be a sequence of i.i.d random variables with $EX_i = \mu$ and $0 < \text{Var}X_i = \sigma^2 < \infty$.
...
Starting from generated uniform i.i.d  random variables, we want methods to transform these into random variables from a target distribution.
